...
def measure_single_traces(
    ...
    print("...")
    time.sleep(3)  # wait for measurement to finish and files to be saved.
...
def measure_train_validation_test(
    ...
    print(alice_control(args="-c -1 -ms 255 -b 1 -da 100 -db 150"))
    print(ram_playback(file_path=train_labels_A_path))  # sets the key repeated by Alice board
    shutil.copy(train_labels_A_path + '.txt', folder_path_PC)
    shutil.copy(train_labels_A_path + '.key', folder_path_PC)

    for suffix in train_suffixes:
        print(f"Measuring training dataset (On, A) {suffix}...")
        ...
        print(f"Training measurement '{suffix}' Done."
              f"Result files:\n{result_files_train}\n")
    input("Disconnect Board to measure in Off state!")

    for suffix in train_suffixes:
        print(f"Measuring training dataset (Off) {suffix}...")
        result_files_train = measure_single_traces(osci, dest_folder_path_on_PC=folder_path_PC,
                                                   folder_path_on_osci=folder_path_osci,
    ...
    shutil.copy(test_labels_A_path+".txt", folder_path_PC)
    shutil.copy(test_labels_A_path+".key", folder_path_PC)
    time.sleep(1)  # wait for the folder to be empty again when files are moved!

    for suffix in test_suffixes:
        print(f"Measuring test dataset (On, A key) {suffix}...")
    ...
    for suffix in test_suffixes:
        print(f"Measuring test dataset (Off) {suffix}...")
        result_files_test = measure_single_traces(osci, dest_folder_path_on_PC=folder_path_PC,
                                                  folder_path_on_osci=folder_path_osci,
...
def measure(
    ...
    if folder_path_on_osci is None:
        folder_path_on_osci = os.path.join(f"/media/lecroy/LCRYADMIN/",
                                           "Documents", "Emsec2d")
    ...
    for meas_loc in meas_locations:
        ...
        measure_train_validation_test(
            ...
            train_labels_A_path=train_labels_A_path,
            train_labels_B_path=train_labels_B_path,
            test_labels_A_path=test_labels_A_path,
            test_labels_B_path=test_labels_B_path,
...
def main(oscilloscope):
    ...
    train_labels_A_path = "random_labels_A"
    train_labels_B_path = "all_same_labels_B"
    test_labels_A_path = "random_labels_A"
    test_labels_B_path = "all_same_labels_B"

    for path in [train_labels_A_path, train_labels_B_path, test_labels_A_path, test_labels_B_path]:
        convert_key_file(path + '.txt', output_path=path + '.key')
